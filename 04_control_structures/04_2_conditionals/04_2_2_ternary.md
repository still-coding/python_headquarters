...
Тернарный оператор — это компактная форма записи `if-else`, которая позволяет записать простое условие в одну строку.
...
Технически можно вкладывать тернарные операторы, но это усложняет чтение:
...
### Тернарный оператор vs обычный if-else
...
Переходите к изучению [[04_3_1_for_loop|цикла for]] для повторения действий.
